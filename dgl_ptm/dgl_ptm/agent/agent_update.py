...
def agent_update(model_graph, model_params, timestep=None, method='default'):
    '''
    agent_update - Updates agent attributes
    '''
    if method == 'capital':
        capital_update(model_graph, model_params, timestep, method=model_params['capital_update_method'])
    elif method == 'theta':
        agent_perception_update(model_graph, model_params, timestep, method=model_params['perception_method'])
