...
In order to configure Sonarcloud analysis [GitHub Action workflow](git@github.com:SDCCA/dgl_ptm/blob/main/.github/workflows/sonarcloud.yml) you must follow the steps below:
...
1. go to [new code definition administration page](https://sonarcloud.io/project/new_code?id=SDCCA_dgl_ptm) and select `Number of days` option
1. To be able to run the analysis:
   1. a token must be created at [Sonarcloud account](https://sonarcloud.io/account/security/)
   1. the created token must be added as `SONAR_TOKEN` to [secrets on GitHub](https://github.com/SDCCA/dgl_ptm/settings/secrets/actions)
